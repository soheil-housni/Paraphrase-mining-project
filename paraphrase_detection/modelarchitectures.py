...
class SBERTPairClassifier(nn.Module):
    def __init__(self, model : SentenceTransformer, fc_sizes : list, dropout : float = 0.1):
        super().__init__()
        self.sbert = model
        self.dropout = dropout
        OUPUT_DIM = 2
        EMB_DIM = self.sbert.get_sentence_embedding_dimension()
        layers = [nn.Linear(EMB_DIM, fc_sizes[0])]
    ...
    def forward(self, x0 : np.ndarray, x1 : np.ndarray):
        emb1 = self.sbert.encode(sentences = x0, convert_to_tensor = True)
        emb2 = self.sbert.encode(sentences = x1, convert_to_tensor = True)
